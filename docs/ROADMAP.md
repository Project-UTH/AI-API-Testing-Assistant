...
> Cập nhật lần cuối: 2026-08-10 (verify Module 6 + Module 7 bằng Playwright E2E thật + người dùng tự tay dùng thử + 1 API demo tự dựng riêng `shop-api-target` có JWT auth; phát hiện + fix nhiều bug thật, quan trọng nhất là **AI sinh test case thiếu field bắt buộc vì `$ref` schema không được resolve** ảnh hưởng gần như mọi spec thực tế — xem chi tiết ở Module 4)
