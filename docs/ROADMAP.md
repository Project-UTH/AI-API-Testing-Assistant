...
> Cập nhật lần cuối: 2026-08-10 (verify Module 6 + Module 7 bằng Playwright E2E thật + người dùng tự tay dùng thử + 1 API demo tự dựng riêng `shop-api-target` có JWT auth; phát hiện + fix nhiều bug thật: `autoIncludedTestCaseIds` mất khi poll `GET /executions/{id}`, thiếu hỗ trợ query parameter, dialog không cuộn được, dropdown quá hẹp, form dependency báo lỗi im lặng, thiếu chỗ xem chi tiết response, lỗi khoá ngoại khi xoá test case/regenerate có `TestResult`, lỗi khoá ngoại khi đổi dependency source do thứ tự flush của Hibernate, thêm tính năng cấu hình xác thực Target API độc lập với import)
